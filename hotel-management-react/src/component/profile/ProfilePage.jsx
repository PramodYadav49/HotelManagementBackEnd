...
const ProfilePage = () => {
    ...
    useEffect(() => {
        const fetchUserProfile = async () => {
            try {
                ...
                console.log('User with Bookings:', userPlusBookings);

                setUser(userPlusBookings.user);
            } catch (error) {
                console.error('Error fetching user profile or bookings:', error);
                setError(error.response?.data?.message || 'Failed to fetch profile or bookings.');
            }
        };
    ...
    return (
        <div className="profile-page">
            {user && <h2>Welcome, {user.name || 'Guest'}</h2>}
                    ...
                    {user?.booking?.length > 0 ? (
                        user.booking.map((booking) => (
                            <div key={booking.id} className="booking-item">
                                ...
                                <img
                                    src={booking.room?.roomPhoto || '/default-room-image.jpg'}
                                    alt="Room"
